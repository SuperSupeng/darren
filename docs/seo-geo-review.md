...
- 后续性能与资料核查：为阶跃四城、SuperAI、Datawhale 案例补入三份公开来源及中英文支持范围，网页、纯文本和结构化引用保持一致。字体和图片加载按浏览器实测进行了精简，详见 [性能检查](performance-review.md) 与 [案例资料核查](case-evidence.md)。
...
- ESLint、类型检查、57 项测试及正式构建通过，另有字体覆盖与生成物完整性检查。已在实际浏览器检查文章、案例与新增原文/订阅入口；上一轮也检查了产品文案和语言切换。
...
6 篇文章的 12 个去重外部引用中，10 个返回 200，OpenAI Harness Engineering 与 Axios 报道返回 403。403 仅表示本次访问受限，没有将它们当作失效链接删除；未发现 404/410。可访问的页面标题与引用相符，但这不等于独立核实每个论断。案例原有 4 个中英文站内文章条目，后续新增 6 个中英文外部材料条目，涉及 3 个不同来源；三个来源均通过独立 HTTP 200 和正文检查。
...
8 个案例中，SuperAI 走访和 Agent 分享已有本人公开文章；本轮进一步补充 SuperAI 官方报告、阶跃四城的组织者复盘和 Datawhale 开源项目贡献记录。微信工坊、Rumata、WAIC 的部分人数与个人职责，以及 AI+X 最终完成的城市数仍适合补充公开依据，具体见 [案例资料核查](case-evidence.md)。保留“约”“近”“接触到”等限定，不把触达人数改成到场人数，也不把组织关系描述成客户背书。

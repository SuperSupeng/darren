...
最初对相同字符集的候选测量中，静态 400、500、600 三份合计 763,464 字节，变量 400–600 为 434,816 字节（均为分拆前、保留所有样式特性的候选）。因此保留可变字体，减少跨字重重复数据；两份文件使用标准排版特性，当前实际体积与完整覆盖范围以 `manifest.json` 和检查命令输出为准。这些历史候选数字不代表当前页面传输量，页面性能仍须通过浏览器测量。

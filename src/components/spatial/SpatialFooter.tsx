...
export default function SpatialFooter() {
  ...
  return <footer className="spatial-footer">
    <div className="spatial-footer-top">
      ...
      <nav className="spatial-footer-map" aria-label={zh ? '网站导航' : 'Site navigation'}>{footerLinks.map(link => <Link key={link.href} href={`${link.href}${lighting === 'evening' ? '?light=evening' : ''}`}>{zh ? link.zh : link.en}</Link>)}</nav>
